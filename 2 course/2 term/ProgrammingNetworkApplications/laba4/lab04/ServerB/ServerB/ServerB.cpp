...
int main()
{
    ...
    /*serv.sin_addr.s_addr = inet_addr("26.220.203.167");*/
    ...
    try {
        while (true) {
            ...
            if (strcmp(name, ibuf) == 0) {
                ...
                cout << "Client IP: " <<  inet_ntoa(client->sin_addr) << " Port: " << ((SOCKADDR_IN*)from)->sin_port << endl;
...
void GetServer(const char* call, short port, struct sockaddr* from, int* flen) {
    ...
    try {
       ...
       /* all.sin_addr.s_addr = INADDR_BROADCAST;*/ // всем 
        //all.sin_addr.s_addr = inet_addr("127.0.0.255"); // всем 
        ...
        if ((lb = sendto(cC, call, sizeof(call), NULL, (sockaddr*)&all, sizeof(all))) == SOCKET_ERROR) {
