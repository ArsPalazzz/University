...
int main(int argc, char* argv[])
{
    ...
    //server.sin_addr.s_addr = inet_addr("26.220.203.167");
    ...
    try
    {
        if (WSAStartup(MAKEWORD(2, 0), &wsaData) != 0)                              throw set_error_msg_text("Startup:", WSAGetLastError());

        GetServer(NAME, 2000, (sockaddr*)&client, &lclient);
        if ((sS = socket(AF_INET, SOCK_DGRAM, NULL)) == INVALID_SOCKET)             throw set_error_msg_text("socket:", WSAGetLastError());
        if (bind(sS, (LPSOCKADDR)&server, sizeof(server)) == SOCKET_ERROR)          throw set_error_msg_text("bind:", WSAGetLastError());

        char serverhostname[128] = "";
        if (gethostname(serverhostname, sizeof(serverhostname)) == SOCKET_ERROR)    throw set_error_msg_text("gethostname:", WSAGetLastError());
        ...
        if (closesocket(sS) == SOCKET_ERROR)             throw set_error_msg_text("closesocket:", WSAGetLastError());
        if (WSACleanup() == SOCKET_ERROR)                throw set_error_msg_text("Cleanup:", WSAGetLastError());
    }
...
bool GetRequestFromClient(char* name, short port, struct sockaddr* from, int* flen) {
    ...
    try {
        while (true) {
            ...
            if (recvfrom(sS, bfrom, sizeof(bfrom), NULL, from, flen) == SOCKET_ERROR) throw set_error_msg_text("GetRequestFromClient recvfrom: ", WSAGetLastError());
    ...
    catch (string errorMsgText)
    {
        ...
        throw set_error_msg_text("GetRequestFromClient:", WSAGetLastError());
    }
}
...
bool PutAnswerToClient(char* name, struct sockaddr* to, int* lto) {
    try
    {
        if ((sendto(sS, name, strlen(name) + 1, NULL, to, *lto)) == SOCKET_ERROR)   throw set_error_msg_text("sendto:", WSAGetLastError());
        return true;
    }
    catch (string errorMsgText)
    {
        throw set_error_msg_text("PutAnswerToClient: ", WSAGetLastError());
    }
}
...
void GetServer(char* call, short port, struct sockaddr* from, int* flen) {
    ...
    try {
        ...
        if (setsockopt(cC, SOL_SOCKET, SO_BROADCAST, (char*)&optval, sizeof(int)) == SOCKET_ERROR)   
            throw  set_error_msg_text("setsocketopt:", WSAGetLastError());
        ...
        //server.sin_addr.s_addr = inet_addr("192.168.157.99");
        //all.sin_addr.s_addr = inet_addr("26.220.203.167");
    ...
    catch (string errorMsgText)
    {
        if (WSAGetLastError() == WSAETIMEDOUT) {
            ...
            if (closesocket(cC) == SOCKET_ERROR) throw set_error_msg_text("closesocket: ", WSAGetLastError());
        }
        else throw set_error_msg_text("GetServer:", WSAGetLastError());
    }
}