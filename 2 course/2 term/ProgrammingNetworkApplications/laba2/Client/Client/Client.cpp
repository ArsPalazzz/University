...
int main()
{
    ...
    try {
        ...
        if ((cC = socket(AF_INET, SOCK_STREAM, NULL)) == INVALID_SOCKET) { //домен, тип и протокол
            throw  set_error_msg_text("socket: ", WSAGetLastError());
        }
        if ((connect(cC, reinterpret_cast<sockaddr*>(&serv), sizeof(serv))) == SOCKET_ERROR) {  // подключение к серверу
