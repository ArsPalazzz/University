...
begin
    ...
    loop
        if mod(to_number(extract(year from teacher.birthday)) - to_number(extract(year from (add_months(sysdate, 12)))), 5) = 0 
            then dbms_output.put_line(teacher.teacher_name||' --- '||teacher.birthday|| '; '|| 'He (She) turns: '||TRUNC((MONTHS_BETWEEN(SYSDATE, teacher.birthday)/12)+1));
        end if;
    end loop;
end;
...
BEGIN
  IF divisor = 0 THEN
    RAISE_APPLICATION_ERROR(-20001, 'Error: division by 0!');
  ELSE
    result := dividend / divisor;
    DBMS_OUTPUT.PUT_LINE('Devision result: ' || result);
  END IF;
EXCEPTION
  WHEN ZERO_DIVIDE THEN
    DBMS_OUTPUT.PUT_LINE('Error: division by 0!');
END;
----------------------------
DECLARE
  dividend NUMBER := 10;
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('The teacher was found: ' || v_teacher_name);
EXCEPTION
  WHEN NO_DATA_FOUND THEN
    DBMS_OUTPUT.PUT_LINE('The teacher was not found!');
END;
