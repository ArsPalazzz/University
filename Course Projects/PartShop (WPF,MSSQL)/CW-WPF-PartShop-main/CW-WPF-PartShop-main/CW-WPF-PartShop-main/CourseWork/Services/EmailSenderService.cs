...
namespace CourseWork.Services
{
    public static class EmailSenderService
    {
        ...
        {
            MailAddress from = new MailAddress("palaznika609@gmail.com", "PartShop");
            MailAddress to = new MailAddress("palaznika608@gmail.com");
            MailMessage message = new MailMessage(from, to);
            message.Subject = "Test";
            message.Body = "Письмо-тест 2 работы smtp-клиента";
            SmtpClient smtp = new SmtpClient("smtp.gmail.com", 587);
            smtp.Credentials = new NetworkCredential("palaznika609@gmail.com", "nxepuparzgbggaor");
            smtp.EnableSsl = true;
        ...
        public static async Task SendCode(string UserMail, int code)
        {
            try
            {
                ...
                SmtpClient smtp = new SmtpClient("smtp.gmail.com", 587);
                smtp.Credentials = new NetworkCredential("palaznika609@gmail.com", "nmahhdrxupwxtary");
                smtp.EnableSsl = true;
                await smtp.SendMailAsync(message);
            }
            catch(Exception e)
        ...
        public static async Task SendCodeRefactor(string UserMail, int code, string subject, string body)
        {
            try
            {
                MailAddress from = new MailAddress("palaznika608@gmail.com", "PartShop");
                MailAddress to = new MailAddress("palaznikarseni@gmail.com");
                MailMessage message = new MailMessage(from, to);
                message.Subject = subject;
                message.Body = body + code.ToString();
                SmtpClient smtp = new SmtpClient("smtp.gmail.com", 587);
                smtp.Credentials = new NetworkCredential("palaznika608@gmail.com", "nxepuparzgbggaor");
                smtp.EnableSsl = true;
                await smtp.SendMailAsync(message);
            }
            catch (Exception e)
        ...
        public static string GenerateTicket(Order order)
        {
            using (PartShopDbContext db = new PartShopDbContext()) 
            {
                ...
                body.Append($"Чек заказа {order.OrderId} из магазина автозапастей Gear Garage");
                body.Append("\n" + order.OrderDate);
                body.Append("\n-----------------------------------");
