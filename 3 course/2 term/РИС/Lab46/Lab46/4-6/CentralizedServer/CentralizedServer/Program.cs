...
namespace CentralizedServer
{
    class Program
    {
        public static string ip = "192.168.96.55";
        ...
        public static void ReplicateDataToCentralDB(string connectionStringName, Data data)
        {
            ...
            {   
                using (UnitOfWork unitOfWork = new UnitOfWork(connectionStringName))
                {
                    ...
                    var data2 = unitOfWork.DataRepository.GetItems();

                    // Репликация данных в FirstDB
                    using (UnitOfWork centralDbUnitOfWork = new UnitOfWork("ServerDBConnect"))
                    {
                        Console.WriteLine("Репликация данных в ServerDB...");
                        foreach (var item in data2)
                        {
                            centralDbUnitOfWork.DataRepository.AddItem(item); // Предположим, что здесь используется метод AddItem() для добавления данных
                        }
                        centralDbUnitOfWork.Save();
                    }
