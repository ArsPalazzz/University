...
class _FoodListScreenState extends State<FoodListScreen> {
  ...
  void didChangeDependencies() {
    ...
    // Получаем доступ к DishBloc в didChangeDependencies
    dishBloc = DishBlocProvider.of(context);
    dishBloc.eventSink.add(LoadDishesEvent());
  }

  @override
  Widget build(BuildContext context) {
    final dishBloc = DishBlocProvider.of(context);
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 20),
                    StreamBuilder<DishState>(
                      stream: dishBloc.stateStream,
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          return Center(child: CircularProgressIndicator());
                        } else if (snapshot.hasError) {
                          return Center(child: Text('Error: ${snapshot.error}'));
                        } else if (snapshot.data is DishLoadingState) {
                          return Center(child: CircularProgressIndicator());
                        } else if (snapshot.data is DishLoadedState) {
                          final state = snapshot.data as DishLoadedState;

                          return Column(
                            children: state.dishes.map((dish) {
                              return Builder(
                                builder: (BuildContext context) {
                                  return Container(
                                    width: MediaQuery.of(context).size.width,
                                                  ...
                                                  IconButton(
                                                    ...
                                                    onPressed: () {
                                                      Navigator.push(
                                                        context,
                                                        MaterialPageRoute(
                                                          builder: (context) => FoodDetailScreen(dish: dish, dishes: state.dishes),
                                                        ),
                                                      );
                                                    },
                                                  ),
