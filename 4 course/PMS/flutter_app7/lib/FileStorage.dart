...
class FileStorage {
  ...
  Future<File> writeGameEntity(GameEntity entity, String filename, String dir) async {
    try {
  ...
  Future<String> readGameEntity(String filename, String dir) async {
    try {
      final file = await _localFile(filename, dir);
  ...
  Future<File> writeEntityList(List<GameEntity> entityList, String filename, String dir) async {
    try {
      final file = await _localFile(filename, dir);
      return await file.writeAsString(jsonEncode(GameEntity.listToJson(entityList)));
    } catch (e) {
      throw Exception('Error writing to file: $e');
    }
  }

  Future<List<GameEntity>> readEntityList(String filename, String dir) async {
    try {
      final file = await _localFile(filename, dir);
      final contents = await file.readAsString();
      final jsonList = jsonDecode(contents) as List<dynamic>;
      return GameEntity.listFromJson(jsonList);
    } catch (e) {
