...
void sortPoAge(Actors myActs[], int all) {
	...
	for (int i = 0; i < all; i++) {
		...
		for (int i = 0; i < all; i++) {
			...
			swap(myActs[smallest_id].index, myActs[i].index);
		}
	}
...
void filt(Actors myAct[], int all) {
	{
		system("cls");
		printf("\t\t\t\t\t\tАктеры\n");
		...
		printf("\nВведите номер человека, в имени которого хотите найти подстроку: ");
		int man;
		scanf_s("%d", &man);
		string s = myAct[man - 1].name;
		...
		cout << "Подстрока " << ps << " находится в позиции ";
		for (i = s.find(ps, i++); i != string::npos; i = s.find(ps, i + 1)) {
			cout << i;
			// после нахождения первого вхождения, i увел. на единицу и find() продолжает поиск со след. символа после найденной головы
			// i = s.find(ps, i++) - первое вхождение, 
			// i != string::npos - пока наш символ не равен символу конца строки, 
			// i = s.find(ps, i + 1) - двигаемся к следующему символу
		}
...
void interPoisk(Actors myAct[], int all) {
	system("cls");
	printf("Все индексы:\n");
...
int Search_Binary(Actors arr[], int left, int right, int key)
{
	int midd = 0;
	while (1)
	{
		...
		if (key < arr[midd].numb)       // если искомое меньше значения в ячейке
			right = midd - 1;      // смещаем правую границу поиска
		else if (key > arr[midd].numb)  // если искомое больше значения в ячейке
...
void main() {
	...
	for (i1 = 0; i1 < allActs; i1++) {
		...
		*(everyAct[i1].index) = rand() % 900000 + 100000;
		everyAct[i1].numb = i1;
	}
	...
	while (gen) {
		...
		case 6:
			...
			printf("Индекс: %d", &everyAct[actforfind].index);
			cin.get(); cin.get();
			break;
		case 7:
			cin.get();
		...
		case 15:
			cin.get();
			filt(everyAct, allActs);
			break;
		case 16:
			cin.get();
			interPoisk(everyAct, allActs);
			break;
		case 17:
			cin.get();
			int key, index;
			cout << "\n\nВведите порядковый номер актера: ";
			cin >> key;
			index = Search_Binary(everyAct, 0, allActs, key);
			if (index >= 0) {
				printf("Имя: %s\n", everyAct[index].name);
				printf("Роль: %s\n", everyAct[index].role);
				printf("Награды: %s\n", everyAct[index].reward);
				printf("Стаж работы: %s\n", everyAct[index].workExp);
				printf("Дополнительно: %s\n", everyAct[index].add);
				printf("Возраст: %d\n", *(everyAct[index].age));
				printf("Индекс: %d\n", *(everyAct[index].index));
